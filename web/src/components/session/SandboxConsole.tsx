  ...
  return (
    <div className="flex flex-col gap-4">
      <div className="flex items-center gap-2">
        ...
        <span className="font-mono text-[12px] text-muted">No real patient data.</span>
      </div>

      <div className="rounded-md border bg-subtle p-4">
        <p className="mb-1 font-mono text-[12px] uppercase tracking-wide text-muted">Scenario</p>
        <p className="text-sm text-primary">{card.scenario}</p>
      </div>

      <div className="flex flex-col gap-2">
        <label htmlFor="sandbox-prompt" className="font-mono text-[12px] uppercase tracking-wide text-muted">
          Your prompt
        </label>
        <textarea
          id="sandbox-prompt"
          ...
          rows={6}
          className="w-full resize-y rounded-md border bg-surface p-3 font-mono text-sm leading-relaxed text-primary outline-none focus-visible:border-strong focus-visible:ring-2 focus-visible:ring-action"
        />
      </div>
      ...
      <div className="flex flex-col gap-2 rounded-md border bg-surface p-4">
        <div className="flex items-center justify-between">
          ...
          <span className="font-mono text-[12px] text-secondary">{rubric.score}/4</span>
        </div>
          ...
          {rubric.dimensions.map((d) => (
            <li key={d.key} className="flex items-start gap-2.5">
              ...
              <span className="flex flex-col">
                <span className={cn("text-sm", d.pass ? "text-primary" : "text-secondary")}>
                  {d.label}
                </span>
                <span className="text-[12px] text-muted">{d.tip}</span>
              </span>
            </li>
          ))}
        </ul>
      ...
      {status === "done" && result && (
        <div className="rounded-md border bg-surface p-4">
          <p className="mb-2 font-mono text-[12px] uppercase tracking-wide text-muted">Model output</p>
      ...
      {status === "done" && notice && (
        <div className="flex items-start gap-2.5 rounded-md border border-info-bg bg-info-bg/40 p-3">
          ...
          <p className="text-sm text-info">{notice}</p>
        </div>
      )}
    </div>
  );
