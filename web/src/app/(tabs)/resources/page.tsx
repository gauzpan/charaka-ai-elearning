  ...
  return (
    <div className="flex flex-col gap-6 animate-card-in">
      <div className="flex flex-col gap-1">
        <p className="font-mono text-[12px] uppercase tracking-wide text-muted">Resources</p>
        <h1 className="font-display text-2xl leading-tight text-primary">Reference shelf</h1>
        <p className="text-secondary">
          The tools and updates worth knowing about — no lessons, just what to reach for.
        </p>
      </div>
        ...
        {items.map((it) => (
          <Link key={it.href} href={it.href} className="block">
            <Card hover className="flex items-center gap-4">
              <span className="grid h-11 w-11 shrink-0 place-items-center rounded-md border bg-subtle text-secondary">
                {it.icon}
              </span>
              <div className="flex flex-1 flex-col gap-0.5">
                <div className="flex items-center gap-2">
                  <h3 className="text-primary">{it.name}</h3>
