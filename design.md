...
> If we build a psychologically informed, workflow-first AI learning platform that offers role-specific journeys, peer coaching, and sandboxed practice — starting with healthcare professionals — then non-tech workers move from *"I get by, but I'm guessing"* to *"I am a confident AI copilot"*, driving sustained adoption, measurable productivity gains, and reduced burnout **without eroding professional identity**.

### 1.2 Who we design for
Time-poor experts with high cognitive load, working in clinical environments, often learning in the gaps between tasks. They are novices at AI but masters of their own field — so the product must respect their expertise while lowering the stakes of being a beginner at *this*.

| Role | Context of use | AI focus example |
|---|---|---|
| Physician | Between patients, on call, commute | Drafting notes, summarizing histories, differential support |
...
Confidence lift (self-reported), time-to-first-real-use of a skill at work, streak retention, and *reduced* effort per task over time. We explicitly **do not** optimize for raw time-in-app (see §10 guardrails).
...
The product should feel less like an LMS and more like a **daily practice with a coach**. Users don't "take a course"; they "do their rounds." This reframing lowers commitment anxiety and supports habit formation.

Three surfaces, always reachable:

- **Learn** — micro-lessons, videos, flashcards, quizzes.
- **Practice (Sandbox)** — a safe, patient-data-free environment to actually use AI prompts and see results, with guardrails and instant feedback.
- **Coach** — asynchronous peer coaching, role peers, and expert answers.
...
- **Reward** — immediate, tangible ("use this in your next note"), plus streak/mastery signal.
...
|---|---|---|
| **Simplicity** | One idea per card; default choices pre-selected; progressive disclosure | Never hide the exit or the "why" |
| **Incentives** | Mastery levels, streaks, competency badges, real-work payoff line on every lesson | Frame as competence, not points-chasing |
...
| **FOMO** | "Your cohort finished this week's round"; streak-at-risk; new role-relevant skill drops | Honest scarcity only; never manufactured guilt |
...
├─ Coach            ← peer threads, ask-an-expert, cohort feed
└─ Library          ← reference/encyclopedia: searchable, task-indexed
...
```
Card types: **Concept**, **Micro-video** (≤ 90s), **Flashcard**, **Mini-quiz**, **Try-it** (sandbox handoff). Authors chunk every lesson so no single screen carries more than one idea.
...
- **Front-load the payoff**: each lesson opens with "By the end you'll be able to ___ in your work."
...
1. Pick role → 2. Pick a real task you want to get better at → 3. Choose your study window (commute / break / evening) → 4. One 2-minute win *now* → 5. Set a gentle daily trigger.
The user finishes their first session before committing to anything. First value precedes first friction.

**Daily microsession.** Open → **Today** shows exactly one next action → play 3–7 min of cards → end on a reward + "try this at work today" + tomorrow queued.

**Mini-quiz.** One question per screen, immediate feedback, explanation on both right and wrong, never punitive. Retries are free.
...
**Streak recovery.** Missing a day offers one low-friction "freeze" or a 60-second make-up card. Recovery is supportive, never shaming.
...
| **Locked** | Show *why* and *when* it unlocks — never a dead end |
...
|---|---|
| Streak | "Daily practice" — consistency, not addiction |
...
| "Currency" | Practice credits that unlock deeper sandbox scenarios — earned by doing, not paying |
...
The visual system is **premium utilitarian minimalism**: warm monochrome canvas, editorial serif for headings, clean geometric sans for UI, muted pastels used *only* for semantic meaning. No gradients, no heavy shadows, no neon, generous whitespace. Clean and quiet so the content leads.
...
| Token | Value | Use |
|---|---|---|
| `--bg-canvas` | `#F7F6F3` | App background (warm bone) |
| `--bg-surface` | `#FFFFFF` | Cards, sheets, session player |
| `--bg-subtle` | `#F9F9F8` | Nested/secondary surfaces |
| `--border` | `#EAEAEA` | All dividers & card borders (1px) |
| `--border-strong` | `rgba(0,0,0,0.10)` | Emphasis dividers |

**Text**
| Token | Value | Use |
|---|---|---|
| `--text-primary` | `#1A1A1A` | Body/headings (never pure black) |
| `--text-secondary` | `#4A4A48` | Supporting text |
| `--text-muted` | `#787774` | Meta, timestamps, hints |

**Interactive**
| Token | Value | Use |
|---|---|---|
| `--action` | `#111111` | Primary button bg |
| `--action-hover` | `#333333` | Hover/pressed |
...
|---|---|---|---|
| Info / interactive accent | `#E1F3FE` | `#1F6C9F` | Tips, "new", info tags, links |
| Success / mastery | `#EDF3EC` | `#346538` | Correct answers, completion |
| Warning / at-risk | `#FBF3DB` | `#956400` | Streak-at-risk, cautions |
| Error / incorrect | `#FDEBEC` | `#9F2F2D` | Errors, wrong answers |

Blue is the single interactive accent so the app reads calm and trustworthy (well suited to a clinical audience) while staying monochrome-dominant.
...
- Streak/badge earn: one restrained celebratory beat, then settle. Never confetti-storms.
...
:root {
  ...
  /* semantic (bg / text) */
  --info-bg: #E1F3FE;      --info-text: #1F6C9F;
  --success-bg: #EDF3EC;   --success-text: #346538;
  --warning-bg: #FBF3DB;   --warning-text: #956400;
  --error-bg: #FDEBEC;     --error-text: #9F2F2D;
  ...
  /* type */
  --font-serif: 'Newsreader','Instrument Serif',serif;
  --font-sans: 'Geist Sans','SF Pro Display',system-ui,sans-serif;
